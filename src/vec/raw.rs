...
impl<T> Vec<T> {
    ...
    pub fn into_iter(self) -> IntoIter<T> {
        IntoIter(self.buckets.into_iter())
    }
    ...
    pub fn clear(&mut self) {
        ...
        for (_, entry) in &mut self.buckets {
            entry.take();
        }
...
pub struct Iter<'a, T>(buckets::Iter<'a, Entry<T>, BUCKETS>);
...
pub struct IntoIter<T>(buckets::IntoIter<Entry<T>, BUCKETS>);
...
impl<T> Iterator for IntoIter<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            let (index, mut entry) = self.0.next()?;
