...
use super::{buckets_index_as_u32, buckets_index_from_u32, BUCKETS, FIRST_OOB_INDEX};
...
impl<K: Key, V> SlotMap<K, V> {
    ...
    pub const fn with_key() -> Self {
        Self {
            buckets: Buckets::new(),
            meta: Meta {
                next_free: AtomicUsize::new(
                    <buckets::Index<BUCKETS>>::new(0).unwrap().into_raw().get(),
                ),
                len: AtomicU32::new(0),
            },
            _key: PhantomData,
    ...
    pub fn try_insert_mut_with<E>(
        ...
        let entry = self.buckets.get_mut_or_alloc(next_free);
    ...
    ///
    /// Panics if the iterator the same index multiple times. Results in dud slots if some but not
    /// all indices in a bucket are given.
    fn from_all_entries_raw<E>(
        ...
        for res in iter {
            ...
            let new_entry = buckets.get_mut_or_alloc(index);
